...
def preprocess_df(mydf, f):
    tmpdf = mydf[[f] + ['case_control', 'BL2AD_yrs', 'Age', 'Gender', 'Education', 'TDI', 'Ethnicity']]
    ...
    tmpdf[['BL2AD_yrs', 'Age', 'Education', 'TDI']] = get_normalization(tmpdf[['BL2AD_yrs', 'Age', 'Education', 'TDI']])
    return tmpdf
...
dpath = '/Volumes/JasonWork/Projects/UKB_DM_Trajectories/Data/AD_Analysis/'
outpath = '/Volumes/JasonWork/Projects/UKB_DM_Trajectories/Results/AD_Analysis/'
...
for f in my_f:
    try:
        ...
        formula = 'Y ~ C(case_control) + BL2AD_yrs + Age + Gender + Education + TDI + C(Ethnicity)'
        p_case = 1
        if nb_levels == 2:
            varType = 'Binary'
...
myout_df['FieldID'] = pd.DataFrame([int(ele.split('-')[0]) for ele in myout_df['FieldID_full'][:391]])
myout_df = pd.merge(myout_df, f_category, how = 'left', on = ['FieldID_full'])
myout_df = pd.merge(myout_df, f_df, how = 'left', on = ['FieldID'])

or_df = pd.read_csv(outpath + 'S0_Results.csv')

or_df = or_df[['FieldID_full',
               'OR_0to5yrs', 'OR_0to3yrs', 'OR_3to5yrs', 'OR_5to6yrs', 'OR_6to7yrs', 'OR_7to8yrs',
               'OR_8to9yrs', 'OR_9to10yrs', 'OR_10to11yrs', 'OR_11to12yrs', 'OR_12to15yrs', 'OR_0to15yrs',
               'p_0to5yrs', 'p_0to3yrs', 'p_3to5yrs', 'p_5to6yrs', 'p_6to7yrs', 'p_7to8yrs',
               'p_8to9yrs', 'p_9to10yrs', 'p_10to11yrs', 'p_11to12yrs', 'p_12to15yrs', 'p_0to15yrs']]
...
reject_case, p_case_fdr11 = fdrcorrection(myout_df.p_0to15yrs.fillna(1))
...
myout_df['p_0to15yrs_fdr'] = pd.DataFrame(p_case_fdr11)

myout_df.to_csv(outpath + 'S1_Results.csv', index=False)

mydf_dict = pd.read_csv(outpath + 'DictTable.csv')
mydf_dict = mydf_dict[['FieldID_full', 'Field_rename', 'Feature_Category_sub', 'Feature_Category', 'Figure2', 'Figure3', 'Figure3_Field']]

myout_df = pd.merge(myout_df,mydf_dict, how = 'left', on = ['FieldID_full'])
myout_df.to_csv(outpath + 'S1_Results.csv', index=False)
