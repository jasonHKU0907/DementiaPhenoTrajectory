...
def preprocess_df(mydf, f):
    tmpdf = mydf[[f] + ['case_control', 'BL2VD_yrs', 'Age', 'Gender', 'Education', 'TDI', 'Ethnicity']]
    ...
    tmpdf[['BL2VD_yrs', 'Age', 'Education', 'TDI']] = get_normalization(tmpdf[['BL2VD_yrs', 'Age', 'Education', 'TDI']])
    return tmpdf
...
for f in my_f:
    try:
        ...
        formula = 'Y ~ C(case_control) + BL2VD_yrs + Age + Gender + Education + TDI + C(Ethnicity)'
        p_case = 1
        if nb_levels == 2:
            varType = 'Binary'
